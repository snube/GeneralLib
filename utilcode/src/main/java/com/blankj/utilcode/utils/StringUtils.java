...
public class StringUtils {
    ...
    public static String getTwoNum(int time) {
        if (time < 10)
            return "0" + time;
        else
            return "" + time;
    ...
    //获取文件夹数组
    public static String[] getPath(String picturesPath) {
        String[] pictures = new String[0];
        if (picturesPath != null && picturesPath.contains("#")) {
            pictures = picturesPath.split("#");
        } else {
            if (picturesPath != null && picturesPath.length() > 0)
                pictures = new String[]{picturesPath};
        }
        return pictures;
    }

    //判断是否是表情
    public static  boolean getLimit(String describle) {
        String reg = "[\\u4e00-\\u9fa5]+";//表示+表示一个或多个中文，
        for (int i = 0; i < describle.length(); i++) {
            String temp=String.valueOf(describle.charAt(i));
            if (!temp.matches(reg)) {
                if (!isEmojiCharacter(describle.charAt(i))) {
                    return false;
                }
         }
        }
    ...
    private static boolean isEmojiCharacter(char codePoint) {
        ...
        return (((int)codePoint >= 0)&& ((int)codePoint <= 177))||codePoint=='。'||codePoint=='，'||codePoint=='；'
                ||codePoint=='？'||codePoint=='！'||codePoint=='（'||codePoint=='）'||codePoint=='《'||codePoint=='》'
                ||codePoint=='“'||codePoint=='”'||codePoint=='”'||codePoint=='、'||codePoint=='’'||codePoint=='：'
                ||(int)codePoint==8212||(int)codePoint==8230||(int)codePoint==12304||(int)codePoint==12305;
    }
    ...
    //获取文件夹数组
    public static ArrayList<String> getListPath(String picturesPath) {
        ArrayList<String> listPath = new ArrayList<>();
        if (picturesPath != null && picturesPath.contains("#")) {
            String[] pictures = picturesPath.split("#");
